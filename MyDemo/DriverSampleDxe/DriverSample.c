  ...
  )
{
  ...
  case EFI_BROWSER_ACTION_FORM_CLOSE:
    {
      ...
      if (QuestionId == 0x2000) {
        ...
        UINTN BufferSize = sizeof (DRIVER_SAMPLE_CONFIGURATION);
        // Status = gRT->GetVariable (VariableName, &gDriverSampleFormSetGuid, NULL, &BufferSize, Configuration);
        HiiGetBrowserData(&gDriverSampleFormSetGuid, VariableName, BufferSize, (UINT8 *)Configuration);
        DEBUG((DEBUG_INFO,"-----%s----\n",Configuration->AddBmcUserPassword));
      ...
      if (QuestionId == 0x2001) {
        ...
        UINTN BufferSize = sizeof (DRIVER_SAMPLE_CONFIGURATION);
        Status = gRT->GetVariable (VariableName, &gDriverSampleFormSetGuid, NULL, &BufferSize, Configuration);
        DEBUG((DEBUG_INFO,"-----%s----\n",Configuration->AddBmcUserPassword1));
        FreePool (Configuration);
      }
    }
    break;
  ...
  case EFI_BROWSER_ACTION_CHANGING:
  {
   ...
   case 0x2003:
      ...
      if (HiiSetString (PrivateData->HiiHandle[0], STRING_TOKEN (STR_TEXT_SAMPLE_HELP), L"1", NULL) == 0) {
        DEBUG((DEBUG_INFO,"--%a--%d--\n",__FUNCTION__,__LINE__));
      }

      if (HiiSetString (PrivateData->HiiHandle[0], STRING_TOKEN (STR_TEXT_SAMPLE_STRING), L"test1", NULL) == 0) {
        DEBUG((DEBUG_INFO,"--%a--%d--\n",__FUNCTION__,__LINE__));
      }
      HiiCreateTextOpCode (
        StartOpCodeHandle,
      ...
      if (HiiSetString (PrivateData->HiiHandle[0], STRING_TOKEN (STR_TEXT_SAMPLE_HELP), L"2", NULL) == 0) {
        DEBUG((DEBUG_INFO,"--%a--%d--\n",__FUNCTION__,__LINE__));
      }

      if (HiiSetString (PrivateData->HiiHandle[0], STRING_TOKEN (STR_TEXT_SAMPLE_STRING), L"test2", NULL) == 0) {
        DEBUG((DEBUG_INFO,"--%a--%d--\n",__FUNCTION__,__LINE__));
      }
      ...
      HiiCreatePasswordOpCode (
        ...
        0,
        0,
        STRING_TOKEN (STR_BMC_USER_PASSWORD),
        STRING_TOKEN (STR_BMC_USER_PASSWORD),
